...
% Lembre-se o octave nao le acento e nem cedilha, por isso, teremos alguns
...
options.vl             = 9;
modelo9 = plsmodel(Xcal,ycal,Xtest,ytest,options);
modelo9.R2c;   % 0.9537
modelo9.R2p    % 0.9500

options.vl             = 6;
modelo6 = plsmodel(Xcal,ycal,Xtest,ytest,options);

modelo6.RMSEC % 1.9061
modelo6.RMSEP % 1.4480
modelo6.R2c   % 0.9614
modelo6.R2p   % 0.9582
...
options.vl             = 4;
modelo4 = plsmodel(Xcal,ycal,Xtest,ytest,options);
...
% anomo-la. Que será identificada com o AAA.

X(AAA,:) = []; y(AAA,:) = []; Sample(AAA,:) = [];
% A amostra 223 é a anomo-la, então devo tira-la de todas partes para não
% atrapalhar minha analise.

% Como se trata de dados de um artigo iremos utilizar a mesma separação
...
% Agora iremos usar um  for, uma função ciclica, que o "qi" ira variar de 1
% a  1 até a quantidade de linhas de X, ou seja, numero de amostras.
for qi=1:1:size(X,1);
...
options.vl             = 15;
modelo15 = plsmodel(Xcal,ycal,Xtest,ytest,options);
...
% avaliacao, todavia, vamos avaliar o grafico de medido e previsto.
...
% Ao observar o gráfico, nota-se pouca diferença entre os modelos a partir
% do 20%. Contudo na faixa de 10% o modelo de VL 15 se mostrou superior,
% vamos para o teste de acuracia.
tic
[pvalue,dist_tt,meandiff] = accuracy_test(ytest,modelo9.Ytest(:,2),modelo15.Ytest(:,2),'randbi',500000,0.05);
toc  %{1068 sec}

% O teste considerou os modelos iguais, utilizando o principio da
% parcimonia, onde menos é mais, escolhemos o modelo com VL 9 como o melhor
% modelo.
